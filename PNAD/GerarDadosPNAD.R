...
escolaridades <- function(pnadc, local_, sexo_, faixa_etaria_, raca_,  contribuicao_, posicao_, atividade_)
{
  # Idade mínima
  pnsub <- subset(pnadc,  pnadc$idade >=14)
...
todas_as_taxas <- function(pnads)
{
  ...
  cat ( paste ("Fonte;", "Local;", "Sexo;", "Faixa etária;", "Raça;", "Previdência;", "Posição;", "Atividade;",
               "População Ativa;", "Pessoas ocupadas;", "Força de trabalho;", "Taxa de desemprego;", "Nível da ocupação;", "Participação;", 
               "Renda habitual;", "Escolaridade", "\n"),
        file = "resultados.csv",
        append = FALSE )
  todas_posicoes = c("")
  todas_atividades = c("", "")
    ...
    for (local in todos_locais)
      for (sexo in todos_sexos)
                ...
                for (atividade in todas_atividades)
        {
          ...
          cat( paste (pnadf, ";", local, ";", sexo, ";", faixa, ";", raca, ";", contribuicao, ";", posicao, ";", atividade, ";",
                      ...
                      escolaridades(pnadc, local, sexo, faixa, raca, contribuicao, posicao, atividade), "\n" ),
               file = "resultados.csv",
               append = TRUE)
        }
  }
